...
public class SortingSpeeds {
  public static void main(String[] args) {
    ...
    do {
      ...
      System.out.println("3) Integer objects using an Arrays.sort()");
      System.out.println("4) Primitive ints using an Arrays.sort()");
      ...
      if(choice != 5) {
        for(int runs = 0; runs < iterations; runs++) {
          ...
          if(choice == 1) {
            ...
            testDescription = "Integer objects use selection sort";
            start = System.nanoTime();
            selectionSortObjects(objArr);
            stop = System.nanoTime();
          }
          else if(choice == 3) {
            testDescription = "Integer objects using Array.sort()";
            start = System.nanoTime();
            Arrays.sort(objArr);
            stop = System.nanoTime();
          }
          else { // choice == 4, probably
            testDescription = "primitive ints using Array.sort()";
            start = System.nanoTime();
            Arrays.sort(intArr);
            stop = System.nanoTime();
          }
